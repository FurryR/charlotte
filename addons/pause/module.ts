...
export const setup = async (addon) => {
    if (vm) {
        return;
    }

    vm = await addon.api.getVM();
    ...
    addon.api
        ...
        .then(() => {
            ...
            const reactInternalKey = Object.keys(soundTab).filter((key) => key.startsWith(addon.api.getReactInternalPrefix()))[0];
